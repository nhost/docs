...
As an example, let's say you have a `company` table and a `company_id` column to the `users` table. With that, you can now add `comppany_id` as a `x-hasura-company-id` key to your JWT claims.
