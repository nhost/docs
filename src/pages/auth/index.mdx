...
**A JWT-token can not be revoked**, they can only expires. Because of that, they are short-lived (15 min). Before a JWT-token expires the client should request a new JWT-token using the refresh-token. **A refresh-token can be revoked** and is long-lived (1 year). To revoke a refresh-token, simply remove the refresh-token from the database in the `auth.refresh_tokens` table.
