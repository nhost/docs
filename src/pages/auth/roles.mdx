...
Each user can have one or more roles. By default, each user has the roles `user` and `me`.
...
If a user is not logged-in and no header or token is sent with the GraphQL request, Hasura resolves the request using the `public` role.

> HASURA_GRAPHQL_UNAUTHORIZED_ROLE is set to `public` by default for all Nhost projects.
