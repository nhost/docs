...
summary: Deploy serverless function with Nhost Custom API. Perfect for Hasura Event triggers and Hasura Actions webhooks.
---
...
```bash
NHOST_HAUSRA_ENDPOINT - (Hasura endpoint)
NHOST_HASURA_ADMIN_SECRET - (Hasura admin secret)
NHOST_HBP_ENDPOINT - (HBP endpoint for auth and storage)
NHOST_CUSTOM_API_ENDPOINT - (Custom API endpoint)
NHOST_WEHOOK_SECRET - (Randoms secret string. Can be used to authenticate requests from Hasura to the Custom API)
```

You can access each environment variable via `process.env.<name>` ex: `process.env.NHOST_HASURA_ENDPOINT`.
