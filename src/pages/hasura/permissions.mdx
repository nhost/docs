...
The GraphQL API is protected by a permission system. The permission system is based on [JWT tokens](https://jwt.io/introduction/). The user gets a JWT token upon login. This JWT token is being sent as a header with all GraphQL API requests to authenticate the user so correct permissions can be applied.
