...
When we login all todos are gone and we can only see **No data**. If we **logout** our todos are back. What's going on?

It's not strange at all. When we are **logged out** we automatically use the `public` role. The `public` role has permission to **select** todos. When we are **logged in** we don't use the `public` role. Instead we use the `user` role. The `user` role does not have permission to **select** todos, yet.
...
Since we are building a secure app we won't use the `public` role. Let's remove all permissions for the `public` role in the Hasura console. Remove both **insert** and **select** permission.
...
First, make sure all old todos are **removed**.

Then go to the **Modify** tab for todos to add a new column. The new column's name is `user_id` (UUID type). Also make sure to **not** click in **nullable**. Then add the column.
...
Last thing we need to do to fully make the connection is for Hasura to track the **foregin key** in the **GraphQL API**. Go to the **DATA** tab and click **Track all**.
