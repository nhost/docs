...
After `create-react-app` finishes, we have to install some packages that will make our lifes easier when connecting to the Nhost backend.
...
`NhostApolloProvider` comes from [react-nhost](https://github.com/nhost/react-nhost) and it deals with all the boilerplate for having `ApolloClient` properly configured to communicate securely with Nhost.

Import `NhostApolloProvider` and use it to wrap your app so that GraphQL is available everywhere.
...
Now, with `ApolloCLient` configured, we can move on to fetching data.
...
Perfect! New todos get listed in realtime when we create them. You can even open two tabs and add new todos in one tab and see how all windows displays the latest data.
