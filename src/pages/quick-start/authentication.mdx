...
A todo should be tied to a user and only be visible to that user. We will now add a new column to our `todos` table **but first we must delete all todos**. Click **Browse Rows** for the `todos` table, select all todos and click on the trash can icon.
...
> All logged in users has the `user` role.
